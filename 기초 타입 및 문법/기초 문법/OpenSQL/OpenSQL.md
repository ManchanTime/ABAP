...
**********************************************************************
* SCARR Table로부터 CARRID, CARRNAME, CURRCODE 데이터를 조회한다.
* 조건 : currcode가 'EUR'인 데이터만 대상으로 조회한다.
...
**********************************************************************
* SFLIGHT Table로부터 CARRID, CONNID를 Unique하게 조회한다.
* 조건 : SEATSOCC가 200보다 큰 데이터를 대상으로 한다.
**********************************************************************
...
* Practice
**********************************************************************
*-- 1. 세금코드(MWSKZ)의 Text table로부터 세금코드 내역(TEXT1)을 읽어와 Internal Table에 세팅한다.
*         TOO7S 선택 조건 : SPRAS EQ sy-langu, KALSM EQ 'TAXKR'
...
*-- 2. 코스트센터(KOSTL)의 Text table로부터 모든 필드들을 읽어와 Internal Table에 세팅한다.
*        CSKT 선택조건 : SPRAS EQ sy-langu KOKRS EQ '0001'
...
*-- 3.SCARR와 동일한 Layout의 Internal Table을 선언하고 Table을 SELECT 하여 Internal Table에 데이터를 저장한다.
*      SELECT 조건 : CARRID 'AA'와 'DL' 사이이의 모든 데이터
...
*-- 3-1. CARRID가 'AA', 'AB', 'CA', 'LH' 데이터를 조회한다.
CLEAR gt_scarr2.
...
*-- 4.SFLIGHT Table에서 아래의 정보를 Select 하여 Internal Table에 저장한다.
*     CARRID, CONNID, FLDATE, PRICE, CURRENY, PLANETYPE, SEATSMAX, SEATSOCC, PAYMENTSUM
*     SELECT 조건 : CARRID가 'AZ', 'QF' AND PLANETYPE이 'A%'인 데이터
...
*-- 5. SBOOK Table에서 아래의 정보를 SELECT 하여 Internal Table에 저장한다.
*       CARRID, CONNID, FLDATE, BOOKID, CUSTOMID, CUSTTYPE, SMOKER, INVOICE, FORCURAM
*       SELECT 조건 : FLDATE가 '2020.10.01' ~ '2020.12.31' 사이의 데이터 AND FORCURAM >= 800
