...
*-- Sub data를 Parameter 조건을 이용해서 조회
  CLEAR : gt_sflight.
...
*-- R3 System인 경우 아래와 같이 DB Table & ITAB 연결 조회
*-- 중복값 제거 필수 -> Backup 테이블이 있어야겠지?
*-- 연결키를 기준으로 SORT한 후 중복값 제거 해야함!!!!!
...
*-- S4 HANA System에서는 DB Table & ITAB 간 INNER JOIN 가능함.
*-- 단, 이와같이 JOIN하게 되면 NEW SQL Syntax 적용 필수!
  CLEAR gt_sflight.
...
**********************************************************************
* 1. SFLIGHT가 Data의 Currency가 'EUR'이면 gs_body-status에 ICON 세팅
* 2. SFLIGHT의 Currency에 따라 gs_body-contry에 대륙명 세팅
* 3. Currency가 'JPY'이면 삭제한다.
**********************************************************************
...
*-- 중복 코드를 제거해 봅시다!
    ...
    CALL METHOD go_alv_grid->set_table_for_first_display
      ...
      CHANGING
        it_outtab       = gt_body
        it_fieldcatalog = gt_fcat.  " 필드 카탈로그는 ITAB 필드랑 같거나 적어야 함! 이름은 반드시 일치할 것!
